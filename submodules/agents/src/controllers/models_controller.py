...
class ModelsController:
    ...
    async def get_available_models(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(
                ...
                headers={"Authorization": f"Bearer {self.mor_api_key}", "Content-Type": "application/json"},
                timeout=30,
            )
            ...
            raise HTTPException(status_code=504, detail="Request to MOR API timed out")
        except requests.RequestException as e:
            logger.error(f"MOR API request failed: {e}")
            raise HTTPException(status_code=502, detail=f"Failed to fetch models from MOR API: {str(e)}")
        except Exception as e:
            logger.error(f"Unexpected error fetching models: {e}")
            raise HTTPException(status_code=500, detail="Internal server error while fetching models")
    ...
    async def get_model_info(self, model_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to fetch model information for {model_id}")

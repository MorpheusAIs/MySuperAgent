...
import { UserMemoriesDB } from '@/services/database/db';
...
export default async function handler(
  ...
  try {
    switch (req.method) {
      case 'GET':
        // Get user memories
        const memories = await UserMemoriesDB.getUserMemories(walletAddress);
      ...
      case 'POST':
        ...
        const newMemory = await UserMemoriesDB.createMemory(
          walletAddress,
          title,
          content
        );
      ...
      case 'PUT':
        ...
        const updatedMemory = await UserMemoriesDB.updateMemory(
          id,
          walletAddress,
          updates
        );
      ...
      case 'DELETE':
        ...
        if (deleteAll === 'true') {
          // Delete all memories
          await UserMemoriesDB.deleteAllMemories(walletAddress);
          return res.status(204).end();
        } else if (memoryId && typeof memoryId === 'string') {
          // Delete specific memory
          await UserMemoriesDB.deleteMemory(memoryId, walletAddress);
          return res.status(204).end();
        } else {
          return res
            .status(400)

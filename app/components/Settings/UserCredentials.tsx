...
export const UserCredentials: React.FC<UserCredentialsProps> = ({ onSave }) => {
  ...
  // Load stored credentials on mount/session change
  useEffect(() => {
  ...
  const handleSaveCredentials = async (serviceName: string) => {
    ...
    try {
      ...
      for (const [credentialName, value] of Object.entries(
        serviceCredentials
      )) {
        if (value.trim()) {
  ...
  const handleDeleteCredentials = async (serviceName: string) => {
    ...
    if (
      !confirm(
        `Are you sure you want to delete all ${serviceName} credentials? This action cannot be undone.`
      )
    ) {
      return;
    }
    ...
    <VStack spacing={4} align="stretch">
      <Box>
        ...
        <Text fontSize="12px" color="rgba(255, 255, 255, 0.6)">
          Securely store API keys and credentials for external services. All
          credentials are encrypted using your wallet signature and stored
          locally.
        </Text>
      </Box>
      ...
      <Grid templateColumns="1fr" gap={3}>
        {SUPPORTED_SERVICES.map((service) => {
          const hasStoredCredentials =
            storedCredentials[service.name]?.length > 0;
            ...
            >
              ...
              <Collapse in={isExpanded}>
                <Box px={3} pb={3}>
                  ...
                  {showCredentials[service.name] &&
                    viewingCredentials[service.name] && (
                      <Box
                        mb={4}
                        p={3}
                      ...
                      >
                        ...
                        <VStack spacing={2} align="stretch">
                          {Object.entries(viewingCredentials[service.name]).map(
                            ([key, value]) => (
                              <Box key={key}>
                                <Text
                                  fontSize="11px"
                                  ...
                                  mb={1}
                                >
                                  {service.fields.find((f) => f.name === key)
                                    ?.label || key}
                                  :
                                </Text>
                                <Box
                  ...
                  <VStack spacing={3} align="stretch">
                    ...
                    <Button
                      ...
                      isDisabled={
                        loading[service.name] || !credentials[service.name]
                      }
                      bg="rgba(255, 255, 255, 0.1)"
                      color="white"
